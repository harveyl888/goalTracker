...
sumOut <- lapply(seq(nrow(df.main)), function(i) {
  ...
  goals <- list(
    ...
    ## Gauges
    knitr::knit_expand(text = "Row {data-height=150}"),
    knitr::knit_expand(text = "-----------------------------\n\n"), # New Row
    sapply(seq(nrow(df.subForMain)), function(j){
      list(
        ...
        knitr::knit_expand(text = sprintf("div(gauge(%s, min = 0, max = 100, symbol = '%%', gaugeSectors(success = c(80, 100), warning = c(40, 79), danger = c(0, 39))), class = \"tooltipX\", span(class = \"tooltiptextX\", \"%s\"))", df.subForMain[j, 'percentComplete'], df.subForMain[j, 'name'])), # Gauge showing progress
        knitr::knit_expand(text = "\n```\n") # end r chunk
      )
    })
  )
